class Login{
    ...
    static endpoint='https://loginv1.gabrielengineer.repl.co/'
    ...
    static login=(callback_ok,callback_naook,config=null)=>{
        ...
        ".elementosLogin{display: flex;justify-content: center;align-items: flex-start;width: 40%;background-color: #eee;flex-direction: column;padding: 10px;border-radius: 10px 0px 0px 10px;}"+
        ...
        `.botoesLogin button{margin: 4px;cursor: pointer;background-color:#${this.config.cor};color: #fff;border-radius: 5px;padding: 10px 20px;width: 100px;box-sizing: inherit;}`
        ...
        labelUsername.setAttribute('id','campoLoginUsername')
    ...
    static verificaLogin=()=>{
        ...
        const endpoint=`https://loginv1.gabrielengineer.repl.co/?matricula=${mat}&senha=${pas}`
            fetch(endpoint)
            .then(res=>res.json())
            .then(res=>{            
                if(res){
